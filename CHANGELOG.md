...
### Fixed
- Added missing non-null assertions (`!`) in `getQueuedItems()`, `getPendingItems()`, `getFailedItems()`, and `getPendingCount()` methods
- These methods now correctly assert database is non-null after the null check
- Fixes runtime `Cannot read property 'query' of null` errors
